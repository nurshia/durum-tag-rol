...
module.exports = {
  name: "sadakat_mod_select",
  async execute(interaction, client) {
    const selectedMod = interaction.values[0];
    const messageId = interaction.customId.split("_").pop();

    updateConfig("sadakatMod", selectedMod);

    const modIsimler = {
      kapali: "❌ Kapalı",
      tag: "🏷️ Sadece Tag",
      durum: "📝 Sadece Durum",
      birlesik: "🔗 Tag VE Durum (Birleşik)",
    };

    await interaction.update({
      content: `✅ Sadakat modu **${modIsimler[selectedMod]}** olarak ayarlandı.`,
      components: [],
    });
